 ...
 */
if (!function_exists('testIpOrDie'))
{
...
{
    $users[] = array();
    $users[] = array("user_id" => 1, "avatar_url" => "http://chat.dev/img/avatar0.png", "name" => "Виктор", "city" => "Москва", "age" => 24, "status" => "active", "login" => "lodin1", "error" => "", "status" => false);
    $users[] = array("user_id" => 2, "avatar_url" => "http://chat.dev/img/avatar0.png", "name" => "Лена", "city" => "Владивосток", "age" => 24, "status" => "active", "login" => "lodin2");
    $users[] = array("user_id" => 3, "avatar_url" => "http://chat.dev/img/avata.png", "name" => "Маша", "city" => "Владивосток", "age" => 0.9, "status" => "active", "login" => "lodin3");
    $users[] = array("user_id" => 4, "avatar_url" => "http://chat.dev/img/avata2.png", "name" => "Михаил", "city" => "Москва", "age" => 30, "status" => "active", "login" => "lodin4");
    $users[] = array("user_id" => 5, "avatar_url" => "http://chat.dev/img/avatar0.png", "name" => "Иван", "city" => "Хабаровск", "age" => 20, "status" => "active", "login" => "lodin5");
    $users[] = array("user_id" => 6, "avatar_url" => "http://chat.dev/img/avatar0.png", "name" => "Артём", "city" => "Артём", "age" => 12, "status" => "active", "login" => "lodin6");
...
function sendUserLoginInfo($user_id)
{
    ...
    if ($output != "usersAuth-ok")
    {
        ...
        echo "http://chat.dev/usersAuth.php?id=" . ((int) $user_id) . "&hash=" . getUserHash($user_id) . "\n";
        var_dump($er);
        var_dump($output);
        echo "</pre>fff";
    }
...
{
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://chat.dev/usersAuth.php?id=" . ((int) $user_id));
